...
if (!class_exists('MassiveCrypto_Shortcodes')) {
    ...
    class MassiveCrypto_Shortcodes {
        ...
        public function ticker_shortcode($id, $options) {
            ...
            if ($options['ticker_design'] == 1) {
                ...
                foreach ($options['data'] as $coin) {
                    ...
                    $output .= ' style="width: '. (strlen($this->number_format($coin->price_usd * $fiatrate, $options['currency'])) + 1) .'ch !important;text-align:center;margin-left:1ch;"';
                    $output .= ">";
                ...
                foreach ($options['data'] as $coin) {
                    ...
                    $output .= ' data-rate="' . $fiatrate . '"';
                    $output .= ' data-currency="' . $options['currency'] . '"';
                    $output .= ' style="width: '. (strlen($this->number_format($coin->price_usd * $fiatrate, $options['currency'])) + 1) .'ch !important;text-align:center;"';
                    $output .= ">";
                    $output .= $this->price_format($coin->price_usd * $fiatrate, $options['currency']);
                    $output .= '</span>';
                    if (in_array('changes',$options['ticker_columns'])) {
        ...
        public function chart_shortcode($id, $options) {
            ...
            $output .= ' data-chartcoin = "'. $options['data'][0]->symbol .'"';
            $output .= ' data-chartcurrency = "'. $options['currency'] .'"';
            $output .= ' data-chartrate = "'. $options['mcw_currencies']->{$options['currency']} .'"';
            $output .= ' data-charttype = "'. $options['chart_type'] .'"';
            $output .= ' data-chartview = "'. $options['chart_view'] .'"';
            $output .= ' data-charttheme = "'. str_replace('custom', '', $options['chart_theme']) .'"';
            $output .= ' data-chartsmooth = "'. $options['chart_smooth'] .'"';
            ...
            $output .= ' data-chartareacolor = "'. $areacolor .'"';
            $output .= ' data-chartbgcolor = "'. $bgcolor .'"></div>';
            $output .= '</div>';
            ...
            if ($id === '') {
                ...
                switch ($atts['info']) {
                    ...
                    case 'marketcap':
                        ...
                        break;
                    case 'change': 
                        $text = '<span>' . $coin->percent_change_24h . '%</span>';
                        break;
                }
