<?php
/**
 * Plugin Name: 		Massive Cryptocurrency Widgets by McTrevor01
 * Description: 		Cryptocurrency widgets for WordPress. Beautifully designed and highly customizable.
 * Author: 				Jephthah Idogbo
 * Author URI:			https://designsbyme.com.ng
 * Requires at least:   4.3.0
 * Requires PHP:        5.6
 * Tested up to:        5.7.2
 * License: 			GPL v3
 * Text Domain:			Massive Crypto Web Widget
 * Domain Path: 		/languages
 * Copyright 2022 McTrevor Exemplaries
**/
...
if (!class_exists('MassiveCrypto')) {
    ...
    class MassiveCrypto {
        ...
        public function display_notices() {
            ...
            switch ($this->options['config']['license_action']) {
                ...
                case 'activate':
                    ...
                    $response = (object)array('license'=>'extended');
                ...
                case 'deactivate':
                    ...
                    $response = (object)array('license_error'=>'site_removed');
        ...
        public function shortcode($atts) {
            ...
            switch ($options['type']) {
                ...
                case 'chart':
                    ...
                    $options['data'] = $this->wpdb->get_results("SELECT `symbol` FROM `{$this->tablename}` " . $wquery);
        ...
        public function get_coinlinks($links, $options) {
            ...
            switch($this->options['config']['linkto']) {
                case 'custom':
                    ...
                    if(!empty($options['config']['link'])){
                        foreach($options['data'] as $coin) {
                            $link = str_replace('[symbol]', strtolower($coin->slug), $options['config']['link']);
                            if(!isset($links[$coin->slug])){
                                $links[$coin->slug] = (parse_url($link, PHP_URL_SCHEME) != '') ? $link : get_site_url(null, $link);
                            }
                        }
                    }
                    ...
                    if (is_plugin_active('coinpress/coinmarketcap.php')) {
                        ...
                        if (!empty($coinmc_config['link'])) {
                            foreach($options['data'] as $coin) {
                                ...
                                $links[$coin->symbol] = (parse_url($link, PHP_URL_SCHEME) != '') ? $link : get_site_url(null, $link);
                                $links[$coin->slug] = (parse_url($link, PHP_URL_SCHEME) != '') ? $link : get_site_url(null, $link);
                            }
                        }
        ...
        public function ajax_tables() {
            ...
            $ordercolumn = (sizeof($options['coins']) > 0) ? "FIELD(`slug`, '" . implode("', '", $options['coins']) . "')" : "rank";
            $ordercolumn = isset($_GET['order'][0]['column']) ? $_GET['columns'][$_GET['order'][0]['column']]['name'] : $ordercolumn;
            $orderdirection = isset($_GET['order'][0]['dir']) ? strtoupper($_GET['order'][0]['dir']) : "ASC";
            ...
            if (sizeof($options['coins']) > 0) {
                ...
                $order = ($ordercolumn == 'slug') ? "FIELD(`slug`, '" . implode("', '", $coins) . "')" : "{$ordercolumn} {$orderdirection}";
                $query = "SELECT `img`, `slug`, `symbol`, `" . implode("`, `", $dbcolumns) . "` FROM `{$this->tablename}` WHERE `slug` IN ('" . implode("', '", $coins) . "') ORDER BY {$order}";
            } else {
                $query = "SELECT `img`, `slug`, `symbol`, `" . implode("`, `", $dbcolumns) . "` FROM `{$this->tablename}` WHERE `rank` <= {$options['numcoins']} ORDER BY `{$ordercolumn}` {$orderdirection} LIMIT " . $_GET['start'] . ", " . $_GET['length'];
            }
